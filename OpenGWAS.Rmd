...
meta <- meta %>%
  dplyr::mutate(genesout_path = ifelse(meta$id %in% names(gene_files2$genes.out), as.character(gene_files2$genes.out[meta$id]), NA)
                ) %>%
  dplyr::mutate(genesout_url = ifelse(is.na(genesout_path),NA,
                                     file.path(base_url,basename(dirname(genesout_path)),basename(genesout_path))))
